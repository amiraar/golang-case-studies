...
func TestHabitStore_ToggleCheckIn(t *testing.T) {
	...
	if !ok || !done {
		t.Fatalf("first toggle: got (done=%v, ok=%v), want (true, true)", done, ok)
	}
	if !s.IsCheckedIn(h.ID, today) {
		t.Error("expected IsCheckedIn true after toggle")
	}

	done, ok = s.ToggleCheckIn(h.ID, today)
	if !ok || done {
		t.Fatalf("second toggle: got (done=%v, ok=%v), want (false, true)", done, ok)
	}
}
...
func TestHabitStore_Streak(t *testing.T) {
	...
	if got := s.Streak(h.ID); got != 0 {
		t.Fatalf("fresh habit streak = %d, want 0", got)
	}

	// Check-in hari ini + 2 hari sebelumnya berturut-turut -> streak 3.
	now := time.Now()
	for i := 0; i < 3; i++ {
...
func TestHabitStore_Delete(t *testing.T) {
	...
	if _, ok := s.Get(h.ID); ok {
		t.Error("expected habit to be gone")
	}
}
